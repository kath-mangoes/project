...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST['resident_id']) || !is_numeric($_POST['resident_id'])) {
        header("Location: complains.php?error=1&message=" . urlencode("Resident ID is required"));
        exit();
    }
    ...
    // Default values
    $actionTaken = null; // Will be filled by barangay officer
    ...
    if (isset($_FILES['document_path']) && !empty($_FILES['document_path']['name'][0])) {
        $upload_dir = "../dist/assets/images/uploads/complaint-documents/";
    ...
    if (empty($complainant) || empty($respondent) || empty($natureOfCase)) {
        header("Location: complains.php?error=1&message=" . urlencode("Please fill all required fields"));
        exit();
    }

    // Insert request into tbl_compgriev
    $insertQuery = "INSERT INTO tbl_compgriev (res_id, user_id, dateFiled, caseNumber, complainant, 
                   respondent, victim, witness, natureOfCase, actionTaken, brgyOfficer_id, 
    ...
    if ($insertStmt->execute()) {
        ...
        $details = "Complain Request Submitted";
        ...
        $audit_status = "Complain Request Created";
        ...
        header("Location: complains.php?success=1");
        exit();
    } else {
        header("Location: complains.php?error=3&message=" . urlencode("Database error: " . $conn->error));
        exit();
    }
    $insertStmt->close();
}
