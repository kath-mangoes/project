...
if (!isset($_GET['format']) || ($_GET['format'] != 'pdf' && $_GET['format'] != 'excel')) {
    header("Location: population.php?error=1");
    exit();
}
...
// SQL query to fetch only PWD residents
$where_clause = "r.is_pwd = 'Yes'";
...
function exportPDF($result, $currentDate) {
    ...
    // Add a title page
    $mpdf->AddPage();
    $mpdf->SetTitle('PWD Residents Report');
    ...
    <div style="text-align: center; padding: 20px;">
        ...
        <h2>PWD Residents Information Report</h2>
...
function exportExcel($result, $currentDate) {
    ...
    $spreadsheet->getProperties()
        ...
        ->setTitle('PWD Residents')
        ->setSubject('PWD Residents Report')
        ->setDescription('PWD Residents information from Barangay 400 Management System');
    ...
    $sheet->setCellValue('C2', 'PWD RESIDENTS INFORMATION REPORT');
    ...
    $headers = [
        ...
        'M' => 'Voter', 'N' => 'Blood Type', 'O' => 'Height', 'P' => 'Weight',
        'Q' => 'Senior', 'R' => '4Ps', 'S' => 'Account Status'
    ];
    ...
    while ($data = $result->fetch_assoc()) {
        ...
        $sheet->setCellValue('M' . $row, $data['is_registered_voter']);
        $sheet->setCellValue('N' . $row, $data['bloodType']);
        $sheet->setCellValue('O' . $row, $data['height']);
        $sheet->setCellValue('P' . $row, $data['weight']);
        $sheet->setCellValue('Q' . $row, $data['is_senior']);
        $sheet->setCellValue('R' . $row, $data['is_4ps_member']);
        $sheet->setCellValue('S' . $row, $data['account_status']);
    ...
    // Borders
    $sheet->getStyle('A' . ($headerRow + 1) . ':S' . ($row - 1))->applyFromArray([
        'borders' => ['allBorders' => ['borderStyle' => Border::BORDER_THIN]]
    ]);
    ...
    $sheet->setTitle('PWD Residents');

    // Output
    $filename = 'Barangay400_PWD_Residents_' . $currentDate . '.xlsx';
