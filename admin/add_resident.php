...
if ($_SERVER["REQUEST_METHOD"] !== "POST") {
    die("Form was not submitted via POST.");
}
...
function getNextUserId($conn) {
    $query = "SELECT user_id FROM tbl_user ORDER BY user_id ASC";
    ...
    if ($result) {
        while ($row = $result->fetch_assoc()) {
            // Only collect IDs that match format like "U001"
            if (preg_match('/^U\d{3}$/', $row['user_id'])) {
                $existingIds[] = intval(substr($row['user_id'], 1));
            }
        }
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($residentStatus)) $errors[] = "Resident status is required";
    if (empty($voterStatus)) $errors[] = "Voter status is required";
    ...
    try {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
            ...
            if (!file_exists($upload_dir)) {
                if (!mkdir($upload_dir, 0777, true)) {
                    throw new Exception("Failed to create upload directory for profile images");
                }
            }
            ...
            if (in_array(strtolower($file_ext), $allowed_ext)) {
                ...
                throw new Exception("Invalid file format for profile image. Only JPG, JPEG and PNG are allowed.");
            }
        }
        ...
        if (isset($_FILES['id_image']) && $_FILES['id_image']['error'] == 0) {
            ...
            if (!file_exists($upload_dir)) {
                if (!mkdir($upload_dir, 0777, true)) {
                    throw new Exception("Failed to create upload directory for ID images");
                }
            }
            ...
            if (in_array(strtolower($file_ext), $allowed_ext)) {
                ...
                throw new Exception("Invalid file format for ID image. Only JPG, JPEG, PNG, and PDF are allowed.");
            }
        }
        
        // Insert into tbl_user
        $insert_user = "INSERT INTO tbl_user (user_id, first_name, middle_name, last_name, password, image, email, mobile, address, account_status, role, terms) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        $stmt->bind_param("sssssssssssi", $user_id, $first_name, $middle_name, $last_name, $hashed_password, $image, $email, $mobile, $address, $account_status, $role, $terms);
        ...
        // Convert height and weight to float if not null
        ...
        // Try to send email, but continue even if email sending fails
        try {
        ...
        // Set success message
        $_SESSION['success'] = "Resident added successfully!" . (!$email_sent ? " However, the welcome email could not be sent." : "");
        ...
        echo $_SESSION['error']; // debug directly
        exit();
