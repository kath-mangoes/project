...
// SQL query to fetch residents data
$sql = "SELECT r.res_id, r.user_id, r.first_name, r.middle_name, r.last_name, 
               ...
               r.address, r.precinctNumber, r.residentStatus, r.voterStatus, 
               r.bloodType, r.height, r.weight, r.typeOfID, r.IDNumber, 
               r.SSSGSIS_Number, r.TIN_number, r.barangay_number, 
               r.is_senior, r.is_pwd, r.is_4ps_member, u.email, u.account_status 
...
function exportPDF($result, $currentDate) {
    ...
    $mpdf->SetTitle('Barangay 400 Residents Report');
    ...
    <div style="text-align: center; padding: 20px;">
        ...
        <h2>Residents Information Report</h2>
        <p>Generated on: ' . date('F j, Y, g:i a') . '</p>
    </div>';
    
    $mpdf->WriteHTML($titlePage);
    
    // Start the actual report content
    $mpdf->AddPage();
    ...
    <div class="header">
        <h1>Barangay 400 Residents Information</h1>
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $html .= '<tr>
            ...
            <td>' . htmlspecialchars($row['voterStatus']) . '</td>
            <td>' . ($row['is_senior'] == 'Yes' ? 'Yes' : 'No') . '</td>
            <td>' . ($row['is_pwd'] == 'Yes' ? 'Yes' : 'No') . '</td>
            <td>' . ($row['is_4ps_member'] == 'Yes' ? 'Yes' : 'No') . '</td>
        </tr>';
        $counter++;
    }
    ...
    // Set the filename
    $filename = 'Barangay400_Residents_' . $currentDate . '.pdf';
...
function exportExcel($result, $currentDate) {
    ...
    $spreadsheet->getProperties()
        ...
        ->setTitle('Barangay 400 Residents')
        ->setSubject('Barangay 400 Residents Report')
        ->setDescription('Residents information generated from Barangay 400 Management System');
    ...
    $sheet->setCellValue('C2', 'RESIDENTS INFORMATION REPORT');
    ...
    $sheet->setCellValue('B' . $headerRow, 'User ID');
    ...
    $sheet->setCellValue('F' . $headerRow, 'Gender');
    $sheet->setCellValue('G' . $headerRow, 'Birthday');
    $sheet->setCellValue('H' . $headerRow, 'Age');
    $sheet->setCellValue('I' . $headerRow, 'Civil Status');
    $sheet->setCellValue('J' . $headerRow, 'Mobile');
    $sheet->setCellValue('K' . $headerRow, 'Email');
    $sheet->setCellValue('L' . $headerRow, 'Address');
    $sheet->setCellValue('M' . $headerRow, 'Resident Status');
    $sheet->setCellValue('N' . $headerRow, 'Voter Status');
    $sheet->setCellValue('O' . $headerRow, 'Precinct #');
    $sheet->setCellValue('P' . $headerRow, 'Blood Type');
    $sheet->setCellValue('Q' . $headerRow, 'Height');
    $sheet->setCellValue('R' . $headerRow, 'Weight');
    $sheet->setCellValue('S' . $headerRow, 'Senior');
    $sheet->setCellValue('T' . $headerRow, 'PWD');
    $sheet->setCellValue('U' . $headerRow, '4Ps Member');
    $sheet->setCellValue('V' . $headerRow, 'Account Status');
    ...
    $sheet->getStyle('A' . $headerRow . ':V' . $headerRow)->applyFromArray($headerStyle);
    ...
    while ($data = $result->fetch_assoc()) {
        ...
        $sheet->setCellValue('N' . $row, $data['voterStatus']);
        $sheet->setCellValue('O' . $row, $data['precinctNumber']);
        $sheet->setCellValue('P' . $row, $data['bloodType']);
        $sheet->setCellValue('Q' . $row, $data['height']);
        $sheet->setCellValue('R' . $row, $data['weight']);
        $sheet->setCellValue('S' . $row, $data['is_senior']);
        $sheet->setCellValue('T' . $row, $data['is_pwd']);
        $sheet->setCellValue('U' . $row, $data['is_4ps_member']);
        $sheet->setCellValue('V' . $row, $data['account_status']);
    ...
    $sheet->getStyle('A' . ($headerRow + 1) . ':V' . ($row - 1))->applyFromArray($borderStyle);
    ...
    // Set filename
    $filename = 'Barangay400_Residents_' . $currentDate . '.xlsx';
